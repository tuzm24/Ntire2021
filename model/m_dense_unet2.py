...
class MegaDenseUnet(nn.Module):
    def __init__(self, args):
        ...
        self.gdu1_1 = GroupDesnseUnet(nChannels=self.base_channel, nDenselayers=self.nDenselayers,
                                    nDenseBlocks=self.nDenseBlocks, growRate=self.growRate)
        self.gdu2 = GroupDesnseUnet(nChannels=self.base_channel, nDenselayers=self.nDenselayers,
                                    nDenseBlocks=self.nDenseBlocks, growRate=self.growRate)
        self.gdu2_2 = GroupDesnseUnet(nChannels=self.base_channel, nDenselayers=self.nDenselayers,
                                    nDenseBlocks=self.nDenseBlocks, growRate=self.growRate)
        self.gdu3 = GroupDesnseUnet(nChannels=self.base_channel, nDenselayers=self.nDenselayers,
                                    nDenseBlocks=self.nDenseBlocks, growRate=self.growRate)
        self.gdu3_3 = GroupDesnseUnet(nChannels=self.base_channel, nDenselayers=self.nDenselayers,
                                    nDenseBlocks=self.nDenseBlocks, growRate=self.growRate)


        self.recon = nn.Sequential(nn.Conv2d(in_channels=self.base_channel * 3, out_channels=self.base_channel, kernel_size=3, stride=1, padding=1, bias=False),
                                   nn.PReLU(),
                                   nn.Conv2d(in_channels=self.base_channel, out_channels=self.base_channel,
                                   ...
                                   nn.Conv2d(in_channels=self.base_channel, out_channels=self.base_channel,
                                             kernel_size=3, stride=1, padding=1, bias=False),
                                   nn.PReLU(),
                                   nn.Conv2d(in_channels=self.base_channel, out_channels=3, kernel_size=3, stride=1, padding=1, bias=False)
                                   )
    ...
    def forward(self,x ):
        ...
        x1 = self.gdu1(x)
        x1 = self.gdu1_1(x1) + x1
        x2 = self.gdu2(x + grid8x8)
        x2 = self.gdu2_2(x2) + x2
        x3 = self.gdu3(x + grid16x16)
        x3 = self.gdu3_3(x3) + x3
        x = torch.cat([x1, x2, x3], dim=1)
        return self.recon(x) + residual
...
if __name__ == '__main__':
    ...
    inpt = torch.randn((7,5,48,48)).cuda()
